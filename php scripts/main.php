...
class Main
{
    ...
    {
        define('conString', 'mysql:host=localhost;dbname=fast-value-inspection');
        define('dbUser', 'root');
        define('dbPass', '');
        $this->dbConnect(conString, dbUser, dbPass);
    }
    ...
    public function registration($username, $pass, $mobile, $usertype)
    {
        ...
        if ($this->checkForExistingMobile($mobile)) {
            return array("status" => "error", "message" => "Mobile number already exists.");
        }
        if (!(isset($username) && isset($pass) && isset($mobile) && isset($usertype))) {
            return array("status" => "error", "message" => 'Insert all valid required fields.');
        }
        ...
        if ($stmt->execute([$username, $pass, $usertype, $mobile])) {
            return array("status" => "success", "message" => "You have registered successfully.");
        } else {
            return array("status" => "error", "message" => 'Inesrting a new user failed.');
        }
    }
